...
import ressources.interactions as intA
...
def genElts_2():
    ...
    # When you get the generator, use intA to generate proper list of elements
...
def genInverses2():
    ...
    config.INVERSIONS_BOX = [invertGalois2(bm.multitype_to_bytes(elt)) for elt in config.ELEMENTS]
    intA.writeVartoFile(config.INVERSIONS_BOX, "inversion_Sbox")
...
def GF2(degree):
    ...
    config.IRRED_POLYNOMIAL = int.from_bytes(bm.multitype_to_bytes(config.IRRED_POLYNOMIAL), "big")
