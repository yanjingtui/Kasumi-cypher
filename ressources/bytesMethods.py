...
#Convert bits arrays, bytes, int, str, b64 to bytes
def multitype_to_bytes(obj:object) -> bytes:
...
#generate number of byte for an int
...
def fileToBytes(file,message=True,directory=config.DIRECTORY_PROCESSING,Verbose=False):
    ...
    True if intA's a .txt file with a message.
    """
    import time
    ...
    if not message: # If intA's a file
...
def output_result(thing, coded=True, inFile=""):
    '''
    ...
    if inFile:
        ...
        file_name = os.path.join(config.DIRECTORY_PROCESSING, inFile)
        ...
        if coded:
            ...
            #result file starting with dec_
            katFile=open(os.path.join(config.DIRECTORY_PROCESSING, f'dec_{os.path.basename(inFile)[:-4]}'),"wb")
            katFile.write(bytes(packed))
...
def byte_operation(b1, b2, ope="XOR"):
    """
...
def hammingWeight(n:object):
    ...
    n = bytes_to_int(multitype_to_bytes(n))
